...
from Units.units import find_maxlen, find_maxlennum, find_char_maxlen, find_single_maxlen, find_distinguish_maxlen
from data.buildBatch import buildBatch, padding_key


def create_tra_batch(tra_data, usual_vat_data, tag_vocab, batch_size, config, tokenizer, domain_vocab, shuffle=False):
    ...
    for batch, vat_batch in zip(unit, vat_unit):
        one_word_data = smp_single_word_data_variable(batch, tag_vocab, domain_vocab, tokenizer, config)
        two_word_data = smp_single_word_data_variable(vat_batch, tag_vocab, domain_vocab, tokenizer, config)
        yield one_word_data, two_word_data


def create_eval_batch(tra_data, tag_vocab, batch_size, config, tokenizer, domain_vocab, shuffle=False):
...
def smp_single_word_data_variable(batch, tag_vocab, domain_vocab, tokenizer, config):
    batch_size = len(batch)
    src_premise_bert_indice = []
    src_premise_segments_id = []
    tag_matrix = np.zeros(batch_size)
    domain_tag_matrix = np.zeros(batch_size)
    data_ids = []
    for idx, instance in enumerate(batch):
        ...
        domain_tag_matrix[idx] = domain_vocab.word2id(instance.topic)
